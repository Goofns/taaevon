...
void main() {
  group('TessellationBoard', () {
    ...
    test('subsequent tiles must be 4-adjacent to the pattern', () {
      ...
      expect(
          TessellationBoard.canPlace(filled, const Cell(0, 1), 3, 3), isTrue);
      expect(
          TessellationBoard.canPlace(filled, const Cell(1, 0), 3, 3), isTrue);
      // diagonal is not 4-connected
      expect(
          TessellationBoard.canPlace(filled, const Cell(1, 1), 3, 3), isFalse);
      // already occupied
      expect(
          TessellationBoard.canPlace(filled, const Cell(0, 0), 3, 3), isFalse);
      // detached from the pattern
      expect(
          TessellationBoard.canPlace(filled, const Cell(2, 2), 3, 3), isFalse);
    });
